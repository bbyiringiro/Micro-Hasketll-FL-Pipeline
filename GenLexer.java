...
class StateOutOfRange extends Exception {
	public StateOutOfRange(String lexClassName, int state) {
		super("Illegal state " + Integer.toString(state) + " in acceptor for " + lexClassName);
	}
}
...
public class GenLexer implements LEX_TOKEN_STREAM {
	...
	LexToken nextToken() throws LexError, StateOutOfRange, IOException {
	...
	public LexToken peekToken() throws LexError, StateOutOfRange, IOException {
		if (bufferInUse) {
			return bufferToken;
		} else {
	...
	public LexToken pullToken() throws LexError, StateOutOfRange, IOException {
		peekToken();
		bufferInUse = false;
		return bufferToken;
	}

	public LexToken peekProperToken() throws LexError, StateOutOfRange, IOException {
	...
	public LexToken pullProperToken() throws LexError, StateOutOfRange, IOException {
		peekProperToken();
		bufferInUse = false;
		return bufferToken;
	}
