...
interface PARSER {
	...
	TREE parseTokenStreamAs(LEX_TOKEN_STREAM tokStream, String nonterm) throws Exception;
}
...
abstract class GenParser implements PARSER {
	...
	public TREE parseTokenStream(LEX_TOKEN_STREAM tokStream) throws Exception {
		return parseTokenStreamAs(tokStream, this.startSymbol());
	}

	public TREE parseTokenStreamAs(LEX_TOKEN_STREAM tokStream, String nonterm) throws Exception {
		...
		if (next != null) {
			...
			System.out.println("Warning: " + next.value() + " found after parse completed.");
		} else {
...
class UnexpectedInput extends Exception {
	public UnexpectedInput(String expected, String found) {
		super("Parse error: " + found + " encountered where " + expected + " expected.");
	}
}
